...
public interface ItemStorageRestExport {
    ...
    @RequestLine("GET /storage/find-all-by-id/{request}")
    FindAllStorageItemsByIdResponse findAllStorageItemsById(@Param("request") List<String> request);

    @RequestLine("GET /storage/user/{userId}")
    CheckUserIfHasOrdersResponse checkIfUserHasOrders(@Param("userId") String userId);
